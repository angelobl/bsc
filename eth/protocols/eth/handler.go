...
func MakeProtocols(backend Backend, network uint64, dnsdisc enode.Iterator, cacheSize uint64) []p2p.Protocol {
	...
	for i, version := range ProtocolVersions {
		...
		protocols[i] = p2p.Protocol{
			...
			Run: func(p *p2p.Peer, rw p2p.MsgReadWriter) error {
				peer := NewPeer(version, p, rw, backend.TxPool(),cacheSize)
				defer peer.Close()
